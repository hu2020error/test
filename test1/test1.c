...
FILE *fp2;	//定义文件指针
fp2=fopen("code","r");	//以读的方式打开文件

admin *s = head;
//读取文件直到为空
while(!feof(fp2)){
admin *p=(admin*)malloc(sizeof(admin));	//定义中间结点接收信息
...
printf("请输入输入用户名：");
scanf("%d",&name);
while(t->next->name != name && t->next != NULL)	//查找相应的用户名
t=t->next;

if(t->next == NULL)	//若没有找到相应的用户名
printf("用户名输入有误！");
else
{
int code1;
printf("请输入密码：");	//找到用户名，输入密码
scanf("%d",&code1);
if(code1==t->next->code)	//若密码正确打开管理员菜单
{
printf("密码正确\n");
//menu2(head2,head);
}
	...
	switch(choose){
	case 1:
	enter(head,head2);	//重新进入管理员登入界面
	break;
	case 0:
	menu1(head2,head);	//打开主菜单
	break;
	//若输入错误命令
	default:
	printf("请输入正确指令\n");
...
//主菜单
void menu1(student *head1, admin *node){
...
//密码入链表
void private1(admin *head){
FILE *fp1;
fp1=fopen("code","r");	//以读的方式打开文件
...
fscanf(fp1,"%d %d ",&p->name,&p->code);	//文件内容输入链表
...
admin *s = (admin*)malloc(sizeof(admin));	//设置存储信息结点
s->next = NULL;

int name;
printf("请输入用户名(数字)：");	//输入用户名
scanf("%d",&name);
s->name=name;

admin *d = head;
while(d->next->name != name && d->next != NULL)
d=d->next;
if(d->next != NULL)
{
printf("用户已经注册\n");
menu1(head2,head);	//返回主菜单
}
...
int code1;
printf("请再输入一遍密码：");	//确认密码
scanf("%d",&code1);
if(code1==code)	//判断两次密码是否一致
{
...
fprintf(fp," %d %d",s->name,s->code);	//把该信息打印至文件
printf("注册成功\n");

private1(head);	//把文件所有内容输入链表
}
fclose(fp);
}
